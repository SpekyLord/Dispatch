...
- Deviations: none.
- Date: `2026-04-05` (bug fix pass #2)
- Completed: fixed 3 TypeScript null-safety errors in `role-news-feed-page.tsx` (`post.location`, `post.photos`, `activeCommentPost.photos` possibly null inside guarded blocks — added non-null assertions); removed 2 unused function declarations in `department-home-page.tsx` (`ProfileField`, `formatRegistryId`); fixed `department-view-page.tsx` empty `.catch()` leaving users stuck in loading state forever — added `fetchError` state with retry UI; fixed `Math.max()` edge case in `municipality-analytics-page.tsx` where empty `by_category` object produced `-Infinity` instead of `1`; rewrote mobile `DepartmentNewsFeedScreen` from hardcoded placeholder to real API-backed feed with realtime subscriptions; replaced dead `DepartmentReportsScreen` placeholder with delegation to `DepartmentReportBoardScreen`; replaced department home page dashboard (web) — removed ~200 lines of hardcoded mock content (fake activity feed, weather widget, map placeholder, readiness stats, news desk, insights) and wired real report data from `/api/departments/reports` with live counts and recent incident links.
- Deviations: none.
...
- Deviations: none.
- Date: `2026-04-05` (security fix)
- Completed: fixed PostgREST filter injection vulnerability in `analytics_service.py`, `reports/routes.py`, and `municipality/routes.py` — user-supplied query parameters (`status`, `category`, `date_from`, `date_to`, `is_escalated`, `verification_status`) were interpolated directly into PostgREST filter strings without sanitization; added `sanitize_postgrest_value()` to `validation.py` that rejects values containing characters that could manipulate PostgREST operators (parentheses, commas, semicolons, spaces).
- Deviations: none.
- Date: `2026-04-05` (analytics fix)
- Completed: fixed municipality analytics dashboard API/frontend field mismatches — web expected `avg_response_time_hours` (single number) but API returned `response_times` (dict): added `_avg_response_hours()` that converts seconds→hours from the first available metric; web expected `unattended_count` but API returned `unattended_reports`: added both keys for mobile compat; web expected `{name, accepts, declines}` for department activity but API returned `{department_id, accepted, declined}`: updated `_compute_department_activity()` to batch-fetch department names and use `accepts`/`declines` keys.
- Deviations: kept `unattended_reports` key alongside `unattended_count` for backward compatibility with mobile.
...
- Date: `2026-04-05` (bug fix)
- Completed: fixed unvalidated `int()` cast on `limit` query parameter in `GET /api/mesh/trail/:deviceFingerprint` — non-integer input (e.g. `?limit=abc`) previously caused an unhandled `ValueError` crash; added `_int_arg()` helper that returns a proper 400 `ApiError` with `validation_error` code, consistent with the existing `_float_arg()` pattern.
- Deviations: none.
